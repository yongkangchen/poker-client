...
public class Lua_NGUITools : LuaObject {
	...
	static public int GammaToLinearSpace_s(IntPtr l) {
		try {
			int argc = LuaDLL.lua_gettop(l);
			if(matchType(l,argc,1,typeof(UnityEngine.Vector4))){
				UnityEngine.Vector4 a1;
				checkType(l,1,out a1);
				var ret=NGUITools.GammaToLinearSpace(a1);
				pushValue(l,true);
				pushValue(l,ret);
				return 2;
			}
			else if(matchType(l,argc,1,typeof(UnityEngine.Color))){
				UnityEngine.Color a1;
				checkType(l,1,out a1);
				var ret=NGUITools.GammaToLinearSpace(a1);
				pushValue(l,true);
